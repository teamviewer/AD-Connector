BeforeAll {
    . "$PSScriptRoot\..\..\Cmdlets\Private\Import-TeamViewerADCConfiguration.ps1"
}

Describe 'Import-TeamViewerADCConfiguration' {
    It 'declares an object output contract' {
        $CommandInfo = Get-Command -Name Import-TeamViewerADCConfiguration
    ...
    It 'retains the source configuration path for later saves' {
        ...
        $ConfigContent = Import-TeamViewerADCConfiguration -Config_File $SourceConfigFile
    ...
    It 'rejects a configuration path that does not exist' {
        ...
        { Import-TeamViewerADCConfiguration -Config_File $MissingConfigFile -ErrorAction Stop } | Should -Throw
    }

    It 'rejects a directory as the configuration path' {
        { Import-TeamViewerADCConfiguration -Config_File $TestDrive -ErrorAction Stop } | Should -Throw
    }
    ...
    It 'propagates malformed JSON errors' {
        ...
        { Import-TeamViewerADCConfiguration -Config_File $SourceConfigFile -ErrorAction Stop } | Should -Throw
    }
