...
Describe 'Split-TVADCUserGroupMemberId' {
    It 'declares pipeline input and its output type' {
        $CommandInfo = Get-Command -Name Split-TVADCUserGroupMemberId
    ...
    It 'uses a default batch size of 100' {
        ...
        $Result = @($InputIds | Split-TVADCUserGroupMemberId)
    ...
    It 'splits pipeline input into ordered batches' {
        ...
        $Result = @($InputIds | Split-TVADCUserGroupMemberId -Size 2)
    ...
    It 'preserves each batch as an ArrayList' {
        $Result = @((1..3) | Split-TVADCUserGroupMemberId -Size 2)
    ...
    It 'rejects a non-positive batch size' {
        { Split-TVADCUserGroupMemberId -InputObject 1 -Size 0 -ErrorAction Stop } | Should -Throw
        { Split-TVADCUserGroupMemberId -InputObject 1 -Size -1 -ErrorAction Stop } | Should -Throw
    }

    It 'rejects null and empty input' {
        { Split-TVADCUserGroupMemberId -InputObject $null -ErrorAction Stop } | Should -Throw
        { Split-TVADCUserGroupMemberId -InputObject '' -ErrorAction Stop } | Should -Throw
    }
}
