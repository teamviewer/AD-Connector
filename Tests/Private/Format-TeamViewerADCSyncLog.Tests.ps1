BeforeAll {
    . "$PSScriptRoot\..\..\Cmdlets\Private\Format-TeamViewerADCSyncLog.ps1"
}

Describe 'Format-TeamViewerADCSyncLog' {
    It 'formats a log entry with its timestamp and message' {
        $Date = [datetime]'2024-01-02 03:04:05'

        $Result = [pscustomobject]@{ Date = $Date; Message = 'Started' } | Format-TeamViewerADCSyncLog
    ...
    It 'formats an empty log message instead of passing the entry through' {
        ...
        $Result = [pscustomobject]@{ Date = $Date; Message = '' } | Format-TeamViewerADCSyncLog
    ...
    It 'uses the current date when a log entry has no date' {
        ...
        $Result = [pscustomobject]@{ Message = 'Started' } | Format-TeamViewerADCSyncLog
    ...
    It 'passes unrelated input through unchanged' {
        ...
        $Result = $InputObject | Format-TeamViewerADCSyncLog
    ...
    It 'rejects null input' {
        { Format-TeamViewerADCSyncLog -InputObject $null -ErrorAction Stop } | Should -Throw
    }

    It 'declares mandatory validated pipeline input' {
        $Parameter = (Get-Command Format-TeamViewerADCSyncLog).Parameters['InputObject']
