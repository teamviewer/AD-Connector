...
Describe 'Resolve-TVADCTeamViewerAccount' {
    BeforeAll {
    ...
    It 'declares required parameters and supports pipeline input' {
        $CommandInfo = Get-Command -Name Resolve-TVADCTeamViewerAccount
    ...
    It 'resolves a user by primary email' {
        ...
        $Configuration = [pscustomobject]@{ Sync_UseSecondaryEmails = $true }

        $Result = $ADUser | Resolve-TVADCTeamViewerAccount -Sync_Context $script:SyncContext -Configuration $Configuration
    ...
    It 'does not use secondary email when disabled' {
        $ADUser = [pscustomobject]@{ Email = 'missing@example.com'; SecondaryEmails = @('secondary@example.com') }
        $Configuration = [pscustomobject]@{ Sync_UseSecondaryEmails = $false }
    ...
    It 'resolves each user supplied through the pipeline' {
        ...
        $Results = @($ADUsers | Resolve-TVADCTeamViewerAccount -Sync_Context $script:SyncContext -Configuration $Configuration)
