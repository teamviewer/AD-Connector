...
Describe 'ConvertTo-TVADCSyncUpdateUserChangeset' {
    It 'returns an empty changeset when either user is missing' {
        (ConvertTo-TVADCSyncUpdateUserChangeset -TV_User $null -AD_User ([pscustomobject]@{})).Count | Should -Be 0
        (ConvertTo-TVADCSyncUpdateUserChangeset -TV_User ([pscustomobject]@{}) -AD_User $null).Count | Should -Be 0
    }
    ...
    It 'returns an empty changeset when users are unchanged' {
        ...
        (ConvertTo-TVADCSyncUpdateUserChangeset -TV_User $TV_User -AD_User $AD_User).Count | Should -Be 0
    }
    ...
    It 'includes a changed name' {
        ...
        $Result = ConvertTo-TVADCSyncUpdateUserChangeset -TV_User $TV_User -AD_User $AD_User
