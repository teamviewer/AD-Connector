BeforeAll {
    . "$PSScriptRoot\..\..\Cmdlets\Private\Add-TVADCSyncLogLine.ps1"
}

Describe 'Add-TVADCSyncLogLine' {
    It 'declares a hashtable output type' {
        $Command = Get-Command Add-TVADCSyncLogLine
    ...
    It 'emits a timestamped log record with the message and extra data' {
        ...
        $Before = Get-Date
        $Result = 'Test message' | Add-TVADCSyncLogLine -Extra $Extra
        $After = Get-Date
    ...
    It 'accepts multiple messages from the pipeline' {
        $Results = 'First', 'Second' | Add-TVADCSyncLogLine
    ...
    It 'rejects an empty message' {
        { Add-TVADCSyncLogLine -Message '' -ErrorAction Stop } | Should -Throw
    }

    It 'rejects a null message' {
        { Add-TVADCSyncLogLine -Message $null -ErrorAction Stop } | Should -Throw
    }
}
