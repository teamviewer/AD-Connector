BeforeAll {
    . "$PSScriptRoot\..\..\Cmdlets\Private\Get-TeamViewerADCScheduledTaskLogDirectory.ps1"
}

Describe 'Get-TeamViewerADCScheduledTaskLogDirectory' {
    BeforeEach {
    ...
    It 'declares a string output contract' {
        $CommandInfo = Get-Command -Name Get-TeamViewerADCScheduledTaskLogDirectory
    ...
    It 'returns the default log directory when no task is supplied' {
        $Result = Get-TeamViewerADCScheduledTaskLogDirectory
    ...
    It 'returns the default log directory for a null task' {
        $Result = Get-TeamViewerADCScheduledTaskLogDirectory -Task $null
    ...
    It 'returns the default log directory when the task has no action arguments' {
        ...
        $Result = Get-TeamViewerADCScheduledTaskLogDirectory -Task $Task
    ...
    It 'returns the log directory configured in the task action arguments' {
        $Task = [pscustomobject]@{
            Actions = @([pscustomobject]@{
                    Arguments = "-NoProfile -Log_Directory 'C:\Configured Logs'"
                })
        }
    ...
    It 'returns the default directory for an empty configured path' {
        $Task = [pscustomobject]@{
            Actions = @([pscustomobject]@{ Arguments = "-Log_Directory ''" })
        }
    ...
    It 'examines only the first task action' {
        $Task = [pscustomobject]@{
            Actions = @(
                ...
                [pscustomobject]@{ Arguments = "-Log_Directory 'C:\Second Action Logs'" }
            )
        }
    ...
    It 'processes each task received from the pipeline' {
        $ConfiguredTask = [pscustomobject]@{
            Actions = @([pscustomobject]@{
                    Arguments = "-Log_Directory 'C:\Configured Logs'"
                })
        }
        $UnconfiguredTask = [pscustomobject]@{ Actions = @() }

        $Result = @($ConfiguredTask, $UnconfiguredTask | Get-TeamViewerADCScheduledTaskLogDirectory)
