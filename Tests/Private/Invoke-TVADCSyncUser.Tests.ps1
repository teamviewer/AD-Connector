BeforeAll {
    ...
    . "$PSScriptRoot\..\..\Cmdlets\Private\ConvertTo-TVADCSyncUpdateUserChangeset.ps1"
    ...
    . "$PSScriptRoot\..\..\Cmdlets\Private\Invoke-TVADCSyncUser.ps1"
}

Describe 'Invoke-TVADCSyncUser' {
    BeforeEach {
    ...
    It 'declares the expected advanced function contract' {
        $CommandInfo = Get-Command -Name Invoke-TVADCSyncUser
    ...
    It 'creates missing users with configured optional values' {
        ...
        $Configuration = [pscustomobject]@{
            ...
            User_Language          = 'de'
            User_MeetingLicenseKey = 'meeting-key'
            Sync_DeactivateUsers   = $false
        }

        $Result = Invoke-TVADCSyncUser -Sync_Context $Sync_Context -Configuration $Configuration -Progress { }
    ...
    It 'updates changed users and counts unchanged users' {
        ...
        $Configuration = [pscustomobject]@{ Api_Token = 'test-token'; TestRun = $false; Sync_DeactivateUsers = $false }
        Mock Resolve-TVADCTeamViewerAccount {
            $script:ResolveCall++
    ...
    It 'deactivates unknown active users but skips the API token owner' {
        ...
        $Configuration = [pscustomobject]@{ Api_Token = 'test-token'; TestRun = $false; Sync_DeactivateUsers = $true }
    ...
    It 'reports planned changes without calling mutation APIs during a test run' {
        ...
        $Configuration = [pscustomobject]@{ Api_Token = 'test-token'; TestRun = $true; Sync_DeactivateUsers = $true }
    ...
    It 'counts failed create, update, and deactivation operations' {
        ...
        $Configuration = [pscustomobject]@{ Api_Token = 'test-token'; TestRun = $false; Sync_DeactivateUsers = $true }
        Mock Resolve-TVADCTeamViewerAccount { $ExistingUser }
