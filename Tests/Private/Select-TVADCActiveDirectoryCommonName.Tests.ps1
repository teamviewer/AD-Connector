...
Describe 'Select-TVADCActiveDirectoryCommonName' {
    It 'declares correct output type' {
        $CommandInfo = Get-Command -Name Select-TVADCActiveDirectoryCommonName
    ...
    It 'extracts CN from simple LDAP distinguished name' {
        ...
        $Result = $DN | Select-TVADCActiveDirectoryCommonName
    ...
    It 'accepts named parameter input' {
        ...
        $Result = Select-TVADCActiveDirectoryCommonName -AD_Path $DN
    ...
    It 'processes multiple pipeline inputs' {
        ...
        $Results = $DNs | Select-TVADCActiveDirectoryCommonName
    ...
    It 'rejects null input with validation error' {
        { Select-TVADCActiveDirectoryCommonName -AD_Path $null -ErrorAction Stop } | Should -Throw
    }

    It 'rejects empty string with validation error' {
        { Select-TVADCActiveDirectoryCommonName -AD_Path '' -ErrorAction Stop } | Should -Throw
    }

    It 'extracts CN from GC (Global Catalog) path' {
        $Path = 'GC://server.example.com/CN=Test Group,OU=Groups,DC=example,DC=COM'

        $Result = $Path | Select-TVADCActiveDirectoryCommonName
