...
function Get-SupportedScheduledSyncInterval() {
    return @(4, 8, 16, 24)
}
...
function Invoke-GraphicalUserInterfaceConfiguration($configuration, [string] $culture) {
    ...
    $scheduledSyncIntervalsData = (Get-SupportedScheduledSyncInterval | ForEach-Object {
    ...
    $mainWindow.FindName('BtnInstallSched').Add_Click( {
            ...
            try {
                Install-ScheduledSync (New-TimeSpan -Hours $mainWindow.DataContext.ScheduledSyncData.Interval) $mainWindow.DataContext.ScheduledSyncData.LogDirectory
            }
            catch {
