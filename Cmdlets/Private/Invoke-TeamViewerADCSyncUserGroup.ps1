...
function Invoke-TeamViewerADCSyncUserGroup {
    ...
    Add-TeamViewerADCSyncLogLine 'Starting user groups synchronization...'
    ...
    Out-TeamViewerADCSyncProgress -Handler $Progress -Percent 80 -Operation 'User Groups'

    # Ensure each configured Active Directory user group has a TeamViewer user group.
    foreach ($AD_UserGroup in $Configuration.ActiveDirectoryGroups) {
        $AD_UserGroupName = ($AD_UserGroup | Select-TeamViewerADCActiveDirectoryCommonName)
        $TV_UserGroup = ($Sync_Context.UserGroups | Where-Object { $_.Name -eq $AD_UserGroupName } | Select-Object -First 1)

        if (-not $TV_UserGroup) {
            Add-TeamViewerADCSyncLogLine "Creating TV user group '$AD_UserGroupName'..."

            if (-not $Configuration.TestRun) {
                try {
                    $TV_UserGroup = (New-TeamViewerUserGroup -ApiToken (ConvertTo-TeamViewerADCSecureString -Value $Configuration.ApiToken) -Name $AD_UserGroupName)

                    $Sync_Statistics.CreatedGroups++
                }
                catch {
                    Add-TeamViewerADCSyncLogLine "Failed to create TV user group '$AD_UserGroupName': $_"
        ...
        $TV_Users = @($AD_Users | Resolve-TeamViewerADCTeamViewerAccount -Sync_Context $Sync_Context -Configuration $Configuration | Where-Object { $_ })
        ...
        foreach ($TV_User in $TV_Users) {
            ...
            if (-not $TV_UserGroupMember) {
                Add-TeamViewerADCSyncLogLine "TV user '$($TV_User.email)' will be added to TV user group '$($TV_UserGroup.name)'..."

                $Members_ToAdd += $TV_User.id.Trim('u')
            }
            else {
                Add-TeamViewerADCSyncLogLine "TV user '$($TV_User.email)' is already member of TV user group '$($TV_UserGroup.name)'. Skipped."

                $Sync_Statistics.NotChanged++
            }
        }

        Add-TeamViewerADCSyncLogLine "Adding TV $($Members_ToAdd.Count) users to TV user group '$($TV_UserGroup.name)'..."

        if (-not $Configuration.TestRun -and $Members_ToAdd.Count -gt 0) {
            $Members_ToAdd | Split-TeamViewerADCUserGroupMemberId | ForEach-Object {
                $TV_CurrentMembersToAdd = $_

                try {
                    (Add-TeamViewerUserGroupMember -ApiToken (ConvertTo-TeamViewerADCSecureString -Value $Configuration.ApiToken) $TV_UserGroup.id $TV_CurrentMembersToAdd) | Out-Null

                    $Sync_Statistics.AddedMembers += $TV_CurrentMembersToAdd.Count
                }
                catch {
                    Add-TeamViewerADCSyncLogLine "Failed to add members to TVuser group '$($TV_UserGroup.name)': $_"
        ...
        foreach ($TV_UserGroupMember in $TV_UserGroupMembers) {
            ...
            if (-not $TV_User) {
                Add-TeamViewerADCSyncLogLine "TV user '$($TV_UserGroupMember.name)' will be removed from TV user group '$($TV_UserGroup.name)'..."

                $Members_ToRemove += $TV_UserGroupMember.accountId
            }
        }

        Add-TeamViewerADCSyncLogLine "Removing $($Members_ToRemove.Count) members from TV user group '$($TV_UserGroup.name)'..."

        if (-not $Configuration.TestRun -and $Members_ToRemove.Count -gt 0) {
            $Members_ToRemove | Split-TeamViewerADCUserGroupMemberId | ForEach-Object {
                $TV_CurrentMembersToRemove = $_

                try {
                    (Remove-TeamViewerUserGroupMember -ApiToken (ConvertTo-TeamViewerADCSecureString -Value $Configuration.ApiToken) $TV_UserGroup.id $TV_CurrentMembersToRemove) | Out-Null

                    $Sync_Statistics.RemovedMembers += $TV_CurrentMembersToRemove.Count
                }
                catch {
                    Add-TeamViewerADCSyncLogLine "Failed to remove members from TV user group '$($TV_UserGroup.name)'"
    ...
    Add-TeamViewerADCSyncLogLine 'Completed user group synchronization.'
