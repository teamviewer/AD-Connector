...
function Invoke-SyncUserGroup($syncContext, $configuration, $progressHandler) {
...
function Invoke-Sync($configuration, $progressHandler) {
    ...
    if ($configuration.EnableUserGroupsSync) {
        Invoke-SyncUserGroup -syncContext $syncContext -configuration $configuration -progressHandler $progressHandler
    }
