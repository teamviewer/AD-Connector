<#
 ...
 .DESCRIPTION
    ...
    Optionally, it also synchronises the AD users to corresponding directory groups that can be used for TeamViewer Conditional Access.
 ...
 .PARAMETER ProgressHandler
    ...
    First, the current progress (percentage). Second, a string identifier of the current action.
 ...
 .PARAMETER LogfileRetentionCount
    The number of log files to keep at max. This is only used when writing to log files is activated. The script cleans-up old log files AFTER the actual sync has taken place. The default value for this parameter is 14, causing two weeks of log files to be kept.
